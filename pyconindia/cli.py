...
def cli(ctx, output_json):
    ...
    if ctx.invoked_subcommand is None:
        ...
        if output_json:
            ...
            click.echo(f"{emoji} Status: {info['status'].replace('_', ' ').title()}")
            
            if info['status'] in ['not_planned', 'prehistoric']:
                click.echo(f"📍 {info['location']}")
                click.echo(f"📝 {info['cfp']}")
            else:
...
def status(ctx, year):
    ...
    conf = Conference()
    result = conf.status(year)
    
    if ctx.obj['json']:
        click.echo(json.dumps({"status": result}))
    else:
        status_emoji = {
            "upcoming": "⏳", 
            "happening": "🔴", 
...
def info(ctx, year):
    ...
    if ctx.obj['json']:
        ...
        click.echo(f"{emoji} Status: {data['status'].replace('_', ' ').title()}")
        
        if data['status'] in ['not_planned', 'prehistoric']:
            click.echo(f"📍 {data['location']}")
            click.echo(f"📝 {data['cfp']}")
...
def city(ctx, year):
    """Get conference city"""
    conf = Conference()
    result = conf.city(year)
    
    if ctx.obj['json']:
        click.echo(json.dumps({"city": result}))
    else:
...
def state(ctx, year):
    ...
    conf = Conference()
    result = conf.state(year)
    
    if ctx.obj['json']:
        click.echo(json.dumps({"state": result}))
    else:
        click.echo(f"🏛️ {result}")
...
def venue(ctx, year):
    ...
    conf = Conference()
    result = conf.venue(year)
    
    if ctx.obj['json']:
        click.echo(json.dumps({"venue": result}))
    else:
        click.echo(f"🏢 {result}")
...
@click.pass_context
def month(ctx, year):
    """Get conference month"""
    conf = Conference()
    result = conf.month(year)
    
    if ctx.obj['json']:
        click.echo(json.dumps({"month": result}))
    else:
        click.echo(f"📅 {result}")
...
def history(ctx, start_year, end_year):
    ...
    if ctx.obj['json']:
        ...
        for y in years:
            ...
            click.echo(f"  {y}: {info['city']}, {info['state']} - {info['venue']}")
